#crosswalks_folder = "c:/Dropbox/Rogerio/Artigos/1 - Em faccao/000 - Padronização de Ocupações e Setores/Rogerio/Harmonization Scripts - INCOMPLETE"
...
harmonize_geography <- function(CensusData, 
                                year,
                                crosswalks_folder = "E:/Dropbox-Ro/Dropbox/Rogerio/Artigos/1 - Em faccao/000 - Padronização de Ocupações e Setores/Rogerio/Harmonization Scripts - INCOMPLETE",
                                delete_originals = T){
        ...
        crosswalk = read_xlsx("crosswalk_states_1960_2010.xlsx") %>%
                filter(year==year) %>%
                as.data.table()
        ...
        if(year == 1960){
                ...
                crosswalk_state <- crosswalk_state %>%
                        filter(year == y) %>%
                        select(-year) %>%
                        setnames(old = "original_code", new = "uf")
        ...
        if( y != 1960 & CensusData[ , max(municipality2010, na.rm = T)] > 999999) {
